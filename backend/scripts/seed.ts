...
//     Its style_profiles rows are built by calling the exact same
//     recordEditAndUpdateStyle() transaction the live app uses — no
//     shortcuts, so the seeded data is provably the same shape as real usage.
...
import { pool, toVectorLiteral } from "../src/lib/db.js";
import { embedText } from "../src/lib/bedrock.js";
import { recordEditAndUpdateStyle } from "../src/lib/memory.js";
...
const HISTORY: Array<{
  news: Parameters<typeof insertNewsItem>[0];
  linkedin: string;
  x: string;
}> = [
  {
...
const BREAKING: Array<Parameters<typeof insertNewsItem>[0]> = [
  {
    source: "Wire",
...
async function seedHistoryForPersona(userId: string) {
  for (const item of HISTORY) {
    const newsItemId = await insertNewsItem(item.news);
    await pool.query(
...
async function main() {
  ...
  await seedHistoryForPersona(ghostId);
  console.log(`seeded ${HISTORY.length} historical stories x 2 platforms for signal_ghost`);

  for (const b of BREAKING) await insertNewsItem(b);
  console.log(`seeded ${BREAKING.length} breaking news items for live triage/draft demo`);
